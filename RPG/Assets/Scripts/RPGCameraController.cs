...
{  
    ...
    private void Update()
    {
        //Change camera position if player's focus has changed
        if (playerController.focus != null && playerController.focus.gameObject.tag == "Enemy" && Player.cl_Player.impulseRifle.activeSelf == true)
        {
            b_IsFocused = true;
            transform.position = focusCamera.transform.position;
            RotateBehindTarget();
        }
    ...
    void LateUpdate ()
    {
        ...
        {
            playerController.animController.SetBool("turnLeft", false);
            playerController.animController.SetBool("turnRight", false);

            //auto-move button pressed
            if (Input.GetButton("Toggle Move") && playerController.b_isAiming == false && !Player.cl_Player.b_IsDeath)
            {
                mouseSideButton = !mouseSideButton;
            }

            //player moved or otherwise interrupted the auto-move.
            if (mouseSideButton && (Input.GetAxis("Vertical") != 0 || Input.GetButton("Jump")) && !Player.cl_Player.b_IsDeath || (Input.GetMouseButton(0) && Input.GetMouseButton(1)) && !Player.cl_Player.b_IsDeath)
            {
            ...
            {
                if (Input.GetMouseButton(0) && !Player.cl_Player.b_IsDeath || Input.GetMouseButton(1) && !Player.cl_Player.b_IsDeath)
                {
                ...
                else if (Input.GetAxis("Vertical") != 0 && !Player.cl_Player.b_IsDeath || Input.GetAxis("Horizontal") != 0 && !Player.cl_Player.b_IsDeath || rotateBehind && !Player.cl_Player.b_IsDeath || mouseSideButton && !Player.cl_Player.b_IsDeath)
                {
                    //Debug.Log(Input.GetAxis("Horizontal"));
                    if (Input.GetAxis("Horizontal") > 0.01f && Input.GetMouseButton(1) == false && !playerController.b_IsFocused)
                    {
                        playerController.animController.SetBool("turnRight", true);
                    }
                    if (Input.GetAxis("Horizontal") < -0.01f && Input.GetMouseButton(1) == false && !playerController.b_IsFocused)
                    {
                        playerController.animController.SetBool("turnLeft", true);
                    }
                    ...
                    if (playerController.moveDirection.z < 0.5f || playerController.moveDirection.z > -0.5f)
                    {
                        RotateBehindTarget();
